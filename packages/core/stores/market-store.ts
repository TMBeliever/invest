...
import type { IndexQuote, MarketSentiment, MarketOverview } from "@investscope/data/schemas";
...
export const useMarketStore = create<MarketState>((set, get) => ({
  ...
  fetchIndices: async () => {
    const key = "indices";
    if (get().loading[key] || isCacheValid(get().lastFetched, key)) return;
  ...
  fetchOverview: async () => {
    ...
    try {
      ...
      set((s) => ({
        loading: { ...s.loading, [key]: false },
        error: { ...s.error, [key]: err instanceof Error ? err.message : "获取市场全景失败" },
      }));
    }
  },
