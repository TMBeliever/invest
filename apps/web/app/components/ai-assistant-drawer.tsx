...
export function AIAssistantDrawer() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className={`fixed right-4 bottom-4 z-50 w-[92vw] sm:w-[420px] glass-panel border border-primary/25 shadow-2xl flex flex-col transition-all duration-300 ${
            isMinimized ? "h-[60px]" : "h-[620px] max-h-[85vh]"
          }`}
        >
          {/* Header */}
          <div className="p-3.5 border-b border-default-100/50 flex items-center justify-between bg-primary/5">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold tracking-tight">InvestScope AI 投资顾问</span>
          ...
          {!isMinimized && (
            <>
                        ...
                        <div
                          className={`p-3 rounded-2xl leading-relaxed whitespace-pre-wrap ${
                            isUser
                              ...
                              : "bg-default-100/80 text-foreground border border-default-200/50 rounded-tl-xs"
                          }`}
                        >
                          {msg.content}
              ...
              {/* Quick Prompts */}
              {messages.length <= 2 && (
                <div className="px-3 py-2 border-t border-default-100/50 bg-default-50/50">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {QUICK_PROMPTS.map((qp, i) => (
                      <button
                        ...
                        disabled={loading}
                        className="text-[10px] px-2 py-1 rounded-lg bg-default-100 hover:bg-primary/15 hover:text-primary border border-default-200/40 text-default-500 transition-all text-left"
                      >
                        {qp}
                      </button>
                    ))}
              ...
              {/* Input Area */}
              <div className="p-3 border-t border-default-100/50 bg-background">
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    handleSend();
                ...
                >
                  <input
                    ...
                    disabled={loading}
                    className="flex-1 px-3 py-2 rounded-xl bg-default-100 text-xs text-foreground placeholder:text-default-400 border border-transparent focus:border-primary focus:outline-none"
                  />
                  <button
                    type="submit"
